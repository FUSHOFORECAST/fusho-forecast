...
INPUT_FILE = "data/processed/model_dataset_regime.csv"
OUTPUT_FILE = "data/processed/forecast_deviation_days.csv"
...
def run_forecast(days=14):
    ...
    for _ in range(days):
        ...
        # baseline dinamica: ultimi 30 giorni disponibili
        baseline = history["total"].tail(30).mean()
        new_row["rolling_30_baseline"] = baseline

        train = history.dropna(subset=["vs_baseline_pct"]).copy()
        ...
        model.fit(train[features], train["vs_baseline_pct"])

        predicted_delta_pct = model.predict(pd.DataFrame([new_row])[features])[0]

        total_pred = baseline * (1 + predicted_delta_pct / 100)

        # share model semplice: usa media ultimi 60 giorni
        recent = history.tail(60)

        delivery_share = recent["delivery_share_target"].mean()
        digital_share = recent["digital_share_target"].mean()
        cash_share = recent["cash_share_target"].mean()
        ...
        results.append({
            "date": future_date,
            "baseline_30": round(baseline, 2),
            "predicted_delta_pct": round(predicted_delta_pct, 2),
        ...
        new_row["vs_baseline_pct"] = predicted_delta_pct
